...
def create_pmr_table(connection):
    ...
    """)
    print(f"Table '{config.PMR_TABLE}' is ready in common database.")

def create_tables(connection):
    cursor = connection.cursor()
    cursor.execute(f"""
        CREATE TABLE IF NOT EXISTS {config.REGIONAL_TABLE} (
            id INT AUTO_INCREMENT PRIMARY KEY, EMPLID VARCHAR(255),
            ...
            RUS_STATUS VARCHAR(255), TOTAL_HOURS DECIMAL(10, 2), Month DATE
        );
    """)
    cursor.execute(f"""
        CREATE TABLE IF NOT EXISTS {config.SALARY_TABLE} (
            id INT AUTO_INCREMENT PRIMARY KEY, EMPLID VARCHAR(255),
            MONTH DATE, GROSS_PAY DECIMAL(10, 2), DESIGNATION VARCHAR(255),
            BAND VARCHAR(255), `FUNCTION` VARCHAR(255)
        );
    """)
    cursor.execute(f"""
        CREATE TABLE IF NOT EXISTS {config.CONSOLIDATION_TABLE} (
            ...
            UNIQUE KEY `unique_employee_month_project` (`EMPLID`,`Month`, `PROJECT_ID`)
        );
    """)
    print(f"Table '{config.REGIONAL_TABLE}', '{config.SALARY_TABLE}', and '{config.CONSOLIDATION_TABLE}' are ready.")
...
def import_regional_details(connection, excel_path):
    ...
    for sheet_name in sheets_to_process:
        ...
        agg_rules = {'TOTAL HOURS': 'sum', 'CURRENT WORK LOCATION': 'first', 'PROJECT DESCRIPTION': 'first', 'PROJECT TYPE': 'first', 'CONTRACT TYPE': 'first', 'CUST NAME': 'first', 'RUS STATUS': 'first'}
        ...
        for _, row in tqdm(df_agg.iterrows(), total=len(df_agg), desc=f"Loading regional {sheet_name}", leave=False):
            sql = f"INSERT INTO {config.REGIONAL_TABLE} (EMPLID, CURRENT_WORK_LOCATION, PROJECT_ID, PROJECT_DESCRIPTION, PROJECT_TYPE, CONTRACT_TYPE, CUST_NAME, RUS_STATUS, TOTAL_HOURS, Month) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
            values = (str(row.get('EMPLID','')).strip(), str(row.get('CURRENT WORK LOCATION','')).strip(), str(row.get('PROJECT ID','')).strip(), str(row.get('PROJECT DESCRIPTION','')).strip(), str(row.get('PROJECT TYPE','')).strip(), str(row.get('CONTRACT TYPE','')).strip(), str(row.get('CUST NAME','')).strip(), str(row.get('RUS STATUS','')).strip(), row.get('TOTAL HOURS'), end_of_month_date)
            cursor.execute(sql, values)

    connection.commit()
    print(f"Regional data loaded into {config.REGIONAL_TABLE}")

def import_salary_data(connection, excel_path):
    xls = pd.ExcelFile(excel_path)
    cursor = connection.cursor()
    for sheet_name in xls.sheet_names:
        ...
        for _, row in tqdm(df.iterrows(), total=len(df), desc=f"Loading salary {sheet_name: <15}", leave=False):
            ...
            sql = f"INSERT INTO {config.SALARY_TABLE} (EMPLID, MONTH, GROSS_PAY, DESIGNATION, BAND, `FUNCTION`) VALUES (%s, %s, %s, %s, %s, %s)"
            values = (str(row.get('EMPLID','')).strip(), end_of_month_date, row.get('GROSS PAY'), str(row.get('DESIGNATION','')).strip(), str(row.get('BAND','')).strip(), str(row.get('FUNCTION','')).strip())
            cursor.execute(sql, values)

    connection.commit()
    print(f"Salary data loaded into {config.SALARY_TABLE}")

def consolidate_data(connection, log_file):
    cursor = connection.cursor()
    cursor.execute(f"TRUNCATE TABLE {config.CONSOLIDATION_TABLE}")
    join_query = f"""
        INSERT INTO {config.CONSOLIDATION_TABLE} (
            EMPLID, Month, GROSS_PAY, DESIGNATION, BAND, `FUNCTION`, CURRENT_WORK_LOCATION,
            PROJECT_ID, PROJECT_DESCRIPTION, PROJECT_TYPE, CONTRACT_TYPE, CUST_NAME,
            PGM_MANAGER_NAME, PGM_MANAGER_EMAIL
        )
        SELECT
            r.EMPLID, r.Month, s.GROSS_PAY, s.DESIGNATION, s.BAND, s.`FUNCTION`,
        ...
        LEFT JOIN {config.COMMON_DB_NAME}.{config.PMR_TABLE} pmr ON r.PROJECT_ID = pmr.PROJECT_ID
    """
    print("Consolidating data via SQL join...")
    cursor.execute(join_query)
    connection.commit()
    print(f"Data consolidated into {config.CONSOLIDATION_TABLE}.")
    
    missing_query = f"""
        SELECT DISTINCT r.PROJECT_ID
        FROM {config.REGIONAL_TABLE} r
        LEFT JOIN {config.COMMON_DB_NAME}.{config.PMR_TABLE} pmr ON r.PROJECT_ID = pmr.PROJECT_ID
        WHERE pmr.PROJECT_ID IS NULL AND r.PROJECT_ID IS NOT NULL AND r.PROJECT_ID != ''
    """
    cursor.execute(missing_query)
    missing_projects = [row[0] for row in cursor.fetchall()]
    with open(log_file, "w") as log:
        log.write("Missing Project IDs (not found in PMR table):\n")
        if missing_projects:
            log.write("\n".join(sorted(missing_projects)))
    print(f"Missing projects logged in {log_file}.")